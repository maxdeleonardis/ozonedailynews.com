...
export const metadata: Metadata = {
  title: 'Science | Research, Discoveries & Breakthroughs',
  description: 'Science news covering physics, biology, climate, astronomy, and research breakthroughs from Ozone Daily News.',
  ...
  twitter: {
    ...
    title: 'Science | Research, Discoveries & Breakthroughs',
    description: 'Science news covering physics, biology, climate, astronomy, and research breakthroughs.',
  },
};
...
export default async function SciencePage() {
  ...
  const articles = all
    .filter((e) => {
      ...
      if (e.slug.startsWith('/science/')) return true;
      if (e.category.toLowerCase() === 'science') return true;
      return false;
    })
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <nav className="text-xs text-gray-500 mb-4">
            ...
            <span className="text-black font-semibold">Science</span>
          </nav>
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-teal-600 shrink-0 mt-1" />
            <div>
              <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-black">Science</h1>
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">
                Physics, biology, climate, astronomy, and the latest research breakthroughs.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        {articles.length === 0 ? (
          <div className="py-24 text-center">
            ...
            <Link href="/" className="mt-4 inline-block text-sm text-teal-600 hover:underline">← Back to home</Link>
          </div>
        ) : (
          <>
            {hero && (
              ...
              >
                {hero.imageUrl ? (
                  ...
                  <div className="w-full sm:w-80 aspect-[16/9] sm:aspect-auto bg-teal-50 border-r border-teal-100 flex items-center justify-center shrink-0">
                    <span className="text-teal-400 text-4xl font-black opacity-40">⬡</span>
                  </div>
                )}
                <div className="p-5 sm:p-6 flex flex-col justify-center flex-1">
                  ...
                  <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-teal-600 self-start mb-3">
                    {hero.category}
                  </span>
                  ...
                  {grid.map((article) => (
                    ...
                    >
                      {article.imageUrl ? (
                        ...
                        <div className="w-full aspect-[16/9] bg-teal-50 border-b border-teal-100 flex items-center justify-center shrink-0">
                          <span className="text-teal-400 text-3xl font-black opacity-30">⬡</span>
                        </div>
                      )}
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-teal-600 self-start mb-2">
                          {article.category}
                        </span>

...
// =============================================================================
// LOCAL-FIRST: reads from content/static/content_registry.json (generated by
// scripts/build-local-registry.ts or the Python build step).
// Supabase is used only as a last-resort fallback when the local file is absent.
// =============================================================================
...
// Sync fallbacks — used when both local file and Supabase are unavailable
import {
...
// ---------------------------------------------------------------------------
// LOCAL FILE REGISTRY — primary source
// ---------------------------------------------------------------------------
...
function loadLocalRegistry(): ContentEntry[] | null {
  ...
  try {
    ...
    console.log(`[registry-service] Loaded ${_localCache.length} entries from local file`);
    return _localCache;
  } catch (e) {
    console.warn('[registry-service] Failed to load local registry:', e);
    return null;
  }
}
...
// ---------------------------------------------------------------------------
// Shared Supabase helper — fallback only
// ---------------------------------------------------------------------------
async function db() {
...
/** All entries, sorted newest first — local file first, Supabase fallback, then empty TS array */
export async function getAllEntries(): Promise<ContentEntry[]> {
  // 1. Try local static file (always available, no network)
  ...
  if (!data || data.length === 0) return _syncRegistry;
...
export async function getEntry(slug: string): Promise<ContentEntry | undefined> {
  ...
  const local = loadLocalRegistry();
  if (local) {
    const found = local.find(e => e.slug === slug);
...
export async function getFeaturedArticles(): Promise<ContentEntry[]> {
  const local = loadLocalRegistry();
  if (local && local.length > 0) return local.filter(e => e.featured);

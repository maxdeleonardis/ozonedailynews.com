...
export default async function Page() {
  ...
  const articles = all
    .filter((e) => {
      const slugMatch = e.slug.startsWith("/shopify/");
      const catMatch = e.category.toLowerCase() === "shopify" || e.category.toLowerCase() === "shopify" || e.category.toLowerCase() === "tech";
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <nav className="text-xs text-gray-500 mb-4">
            ...
            <span className="text-black font-semibold">Shopify</span>
          </nav>
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-green-700 shrink-0 mt-1" />
            <div>
              <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-black">Shopify</h1>
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">Latest Shopify news: agentic storefronts, AI tools, and e-commerce updates.</p>
            </div>
          </div>
        </div>
      </header>
                ...
                >
                  {article.imageUrl ? (
                    ...
                    <div className="w-full aspect-[16/9] bg-green-50 border-green-200 border-b flex items-center justify-center shrink-0">
                      <span className="text-green-700 text-3xl font-black opacity-30">OW</span>
                    </div>
                  )}
                  <div className="p-4 flex flex-col flex-1">
                    <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-green-700 self-start mb-2">
                      {article.category}
                    </span>

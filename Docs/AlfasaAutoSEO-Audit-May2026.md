...
# AlfasaAutoSEO — Phase 2 Audit (Homepage Pipeline)

**Date:** May 13, 2026  
**Trigger:** Homepage showing old/spammy articles instead of new content. New articles from recent commits (GitLab, Tesla FSD, RTX 50, Steam Machine, Forza H6) not appearing correctly in carousel or "More Stories."

---

## Executive Summary

The homepage pulls from three sources and merges them. The merge logic is correct but three systemic bugs cause new articles to be hidden and old/irrelevant content to flood the grid.

**Severity breakdown:**

| Bug | Severity | Status |
|---|---|---|
| `_index.json` read as a fake article | Medium | Open |
| 662 registry entries with hub/dupe pages in "More Stories" | High | Open |
| Malformed slugs in registry (`//events/...`) | Medium | Open |
| New NewsArticle pages have no individual content JSON | High | Open |
| `publish_date` display-format sort instability | Low | Open |

---

## Bug 1 — `_index.json` Treated as an Article (Medium)

**File:** `lib/article-service.ts` → `readStaticDir()`

`readStaticDir('articles')` scans `content/static/articles/*.json` with `filter(f => f.endsWith('.json'))`. This includes `_index.json`. The file is a JSON array; when treated as a `Record<string, unknown>`, all field lookups return undefined.

Result: one garbage article entry per run with `href="/"`, empty title, and `status="published"`. It passes the published filter and pollutes the merge pipeline.

**Fix:**
```typescript
// lib/article-service.ts — readStaticDir helper
.filter(f => f.endsWith('.json') && f !== '_index.json')
```

Apply the same fix to `jack_articles` to be safe.

---

## Bug 2 — Registry Floods "More Stories" With Hub Pages and Duplicates (High)

**File:** `app/page.tsx` — registry filter

The homepage filter excludes registry entries with fewer than 2 path segments. This is insufficient. The registry currently has 662 entries including:

- Sub-hub pages with 2+ path segments (e.g. `/video-games/switch2`, `/nvidia/news`)
- Duplicated articles under two URL structures (e.g. `/video-games/nintendo-mar10-day-2026-40th-anniversary-super-mario` AND `/video-games/nintendo/mar10-day-2026-40th-anniversary-super-mario`)
- Category/index pages that look like articles

These fill the "More Stories" grid with repetitive, non-article links — making the homepage look like a generic sitemap.
...
```typescript
if (e.slug.split('/').filter(Boolean).length < 2) return false;
if (['Meta', 'Services', 'Legal', 'Support', 'Education', 'Reference', 'Blog'].includes(e.category)) return false;
if (e.description.length < 80) return false; // hub pages have short/template descriptions
```

---

## Bug 3 — Malformed Registry Slugs and Encoding Corruption (Medium)

Some entries in `content_registry.json` have:
- Double-slash slugs: `//events/gdc-2026/valve-steam-machine-steam-frame-compatibility-gdc-2026`
- Garbled titles: `"KSI Becomes Co-Owner of Dagenham & Redbridge FC â€" Vows to..."` (encoding bug from em dash → UTF-8 mis-read)

These create broken `<Link href>` elements and corrupted titles in the homepage UI.

**Fix:** Run `npm run wiki:sync -- --write` to rebuild affected entries. Add a validation step in `sync-registry.ts` to reject slugs matching `/^\/\//` and titles containing `â€`.

---

## Bug 4 — New NewsArticle Pages Have No Individual Content JSON (High)

Articles written in the May 9-13 commits that use `NewsArticleDB` and are listed in `content/static/articles/_index.json` do NOT have individual JSON content files:

| Page | `page.tsx` | `_index.json` | Individual `.json` | Content served by |
|---|---|---|---|---|
| GitLab Restructuring May 2026 | ✓ | ✓ | ✗ | Registry only (no content renders) |
| Steam Machine 2026 Launch | ✓ | ✓ | ✗ | Registry only |

`getAllArticles()` calls `readStaticDir('articles')` which reads individual files — NOT `_index.json`. So these articles only appear in the homepage via the registry entry (which has a title and link but no excerpt or image). When a user clicks through to the article, `NewsArticleDB` tries to load the individual JSON file and fails.

**This is the most important bug to fix before publishing more NewsArticles.**

**Fix:** Always create the individual JSON file at `content/static/articles/[slug].json` when writing a NewsArticle. The `_index.json` is a metadata index for tooling only — it does not serve content.

---

## Bug 5 — Date Sort Instability From Display-Format `publish_date` (Low)

`content_registry.json` stores `publish_date` as a display string: `"May 11, 2026"`. This is used by `rowToEntry()` → `publishDate` on the homepage. The sort does `new Date("May 11, 2026").getTime()`.

V8/Node.js generally parses this string correctly, but it is not ISO-8601 and behavior is implementation-defined. On some build environments, this may return NaN, causing comparator instability (articles appear in random order).

**Fix:** Store `publish_date` in the registry as `YYYY-MM-DD` format (already how `sync-registry.ts` sets it for new entries — the issue is legacy entries that were manually set with display format). Run `npm run wiki:sync -- --write` to normalise.

---

## What IS Working Correctly

- JackArticles published via `content/static/jack_articles/*.json` appear correctly. Tesla FSD, RTX 50, Switch 2, Forza H6 all have `publish_date: "May 12, 2026"` and `article_url` set — they route and display correctly.
- Registry entries for GitLab and RTX 50 have `publish_date: "May 11, 2026"` — they DO appear as links on the homepage via the registry path, sorted near the top. But the article pages themselves may 404 if the individual content JSON is missing.
- The dedup merge logic correctly prefers `blogArticles` dates over registry dates.
- The sort correctly pushes null/empty dates to the bottom.

---

## Action Items (Priority Order)

| Priority | Fix | File(s) | Command |
|---|---|---|---|
| 🔴 1 | Create missing individual JSON files for all NewsArticle pages | `content/static/articles/[slug].json` | Manual — write content file |
| 🔴 2 | Fix `readStaticDir()` to skip `_index.json` | `lib/article-service.ts` | Add `&& f !== '_index.json'` filter |
| 🟡 3 | Tighten homepage registry filter | `app/page.tsx` | Add category + description length checks |
| 🟡 4 | Fix malformed registry slugs | `content/static/content_registry.json` | `npm run wiki:sync -- --write` |
| 🟢 5 | Normalise `publish_date` to ISO date format | `content/static/content_registry.json` | `npm run wiki:sync -- --write` |

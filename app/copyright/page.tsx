...
export default async function Page() {
  ...
  const articles = all
    .filter((e) => {
      ...
      const catMatch = e.category.toLowerCase() === "copyright" || e.category.toLowerCase() === "copyright" || e.category.toLowerCase() === "news" || e.category.toLowerCase() === "legal";
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <nav className="text-xs text-gray-500 mb-4">
            ...
            <span className="text-black font-semibold">Copyright</span>
          </nav>
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-gray-700 shrink-0 mt-1" />
            <div>
              <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-black">Copyright</h1>
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">Latest copyright, antitrust and tech legal news: lawsuits, rulings and regulatory updates.</p>
            </div>
          </div>
        </div>
      </header>
              ...
              {articles.map((article) => (
                ...
                >
                  {article.imageUrl ? (
                    ...
                    <div className="w-full aspect-[16/9] bg-gray-50 border-gray-200 border-b flex items-center justify-center shrink-0">
                      <span className="text-gray-700 text-3xl font-black opacity-30">OW</span>
                    </div>
                  )}

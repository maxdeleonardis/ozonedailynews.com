 ...
 *
 * Usage:
 *   npm run wiki:migrate              — migrate all pages
 *   npm run wiki:migrate -- --dry-run — preview without writing
 *   npm run wiki:migrate -- --file app/crypto/xrp/page.tsx — single file
 */
...
// ── Find every content page that still needs migrating ───────────────────────
...
function jsxToHtml(jsx: string): string {
  return jsx
    ...
    // <Link href="..."> → <a href="...">  (handles multi-attribute Link tags too)
    ...
    // {"text"} → text  (simple string expressions)
    .replace(/\{["']([^"'{}]+)["']\}/g, '$1')
...
// ── Extract h1 from the page header ──────────────────────────────────────────
...
// ── Extract category from metadata openGraph section ─────────────────────────
...
// ── Simple string prop extractor (JSX attribute) ─────────────────────────────
...
// ── JS expression prop extractor (array / object props) ──────────────────────
...
// ── Convert DynamicNewsArticle sub-components → HTML ────────────────────────
function convertSubComponents(jsx: string): string {
  ...
  h = h.replace(/<ArticleQuote([\s\S]*?)\/>/g, (_m, p) => {
    ...
    return (
      ...
      ((author || atitle) ? `<footer class="text-sm text-gray-500 mt-2 not-italic">${author ? `— ${author}` : ''}${atitle ? `, ${atitle}` : ''}</footer>` : '') +
      `</blockquote>`
    );
  });
  ...
  h = h.replace(/<CaseOverview([\s\S]*?)\/>/g, (_m, p) => {
    ...
    const rows = items.map(it =>
      `<li class="flex items-start gap-2"><span class="text-${color}-600 font-bold">•</span><span><strong>${it.label}</strong> ${it.value}</span></li>`
    ).join('');
    return (
  ...
  h = h.replace(/<StatisticsBox([\s\S]*?)\/>/g, (_m, p) => {
    ...
    return (
      ...
      `<h4 class="font-bold text-sm mb-3">📊 ${title}</h4>` +
      `<div class="grid ${cols} gap-4 text-center">${rows}</div>` +
      `</div>`
    );
  });

  // KeyFigures → person cards
...
// ── Extract DynamicNewsArticle content ───────────────────────────────────────
function extractDynamicNewsArticleContent(source: string): {
  ...
  const title = rawTitle || extractMetaTitle(source).replace(/\s*[|—–\-].*$/, '').trim() || 'Untitled';
  ...
  // Convert sub-components then apply standard JSX→HTML transforms
...
// ── Extract <NewsArticle> children JSX ───────────────────────────────────────
...
function extractNewsArticleContent(source: string): {
  ...
  // Match <NewsArticle followed by whitespace — avoids matching <NewsArticleSchema
  ...
  // Extract children — everything between closing > of opening tag and </NewsArticle>
...
// ── Parse a freeform publishDate string → ISO timestamp ──────────────────────
...
async function processPage(filePath: string): Promise<'ok' | 'skip' | 'fail'> {
  ...
  if (newsData) {
    ...
    if (isDryRun) {
      console.log(`  ✓  ${relPath} [news article]`);
  ...
  if (dynData) {
    ...
    if (isDryRun) {
      console.log(`  ✓  ${relPath} [dynamic news article]`);
  ...
  // ── PATH 4: Raw JSX layout (SEOWrapper / JackArticle / Hub / bare layout) ─
    ...
    if (returnIdx !== -1) {
      ...
      if (parenEnd !== -1) {
        ...
        if (htmlBody.length < 200) {
          // Too short — likely a stub page, skip
          console.warn(`  ⚠️  ${relPath} — raw JSX body too short (${htmlBody.length} chars), skipping`);
          return 'skip';
        }

        const metaTitle = extractMetaTitle(source).replace(/\s*[|—–\-].*$/, '').trim() || slug;
        const category = extractCategory(source);
        const publishedAt = extractPublishedTime(source);
        ...
        if (isDryRun) {
          console.log(`  ✓  ${relPath} [raw JSX]`);
          console.log(`     slug: ${slug} | title: "${metaTitle}" | author: ${authorName || '—'} | category: ${category}`);
          console.log(`     HTML body length: ${htmlBody.length} chars`);
          return 'ok';
        }
  ...
  // ── No pattern matched ────────────────────────────────────────────────────
  console.warn(`  ⚠️  ${relPath} — no grid or NewsArticle found, skipping`);
  return 'skip';
}
...
async function main() {
  ...
  if (files.length === 0) {
    console.log('⚠️  No NewsArticleSchema pages found.');
    return;
  }
  ...
  console.log(`\n${'─'.repeat(50)}`);
  console.log(`${modeLabel}Done — ${ok} migrated, ${skipped} skipped, ${failed} failed`);

...
export default async function Page() {
  ...
  const articles = all
    .filter((e) => {
      const slugMatch = e.slug.startsWith('/video-games/');
      const catMatch = ['Gaming', 'Video Games', 'video-games'].some(
        (c: string) => e.category.toLowerCase() === c.toLowerCase()
      );
      const tagMatch = e.tags?.some((t: string) =>
        t.toLowerCase().includes('video-games')
      );
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <nav className="text-xs text-gray-500 mb-4">
            ...
            <span className="text-black font-semibold">Video Games</span>
          </nav>
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-violet-700 shrink-0 mt-1" />
            <div>
              ...
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">
                GTA 6, Nintendo Switch 2, gaming industry news and analysis.
              </p>
            </div>
          </div>
        </div>
              ...
              {articles.map((article) => (
                ...
                >
                  {article.imageUrl ? (
                    ...
                    <div className="w-full aspect-[16/9] bg-violet-50 border-violet-200 border-b flex items-center justify-center shrink-0">
                      <span className="text-violet-700 text-3xl font-black opacity-30">OW</span>
                    </div>
                  )}
                  <div className="p-4 flex flex-col flex-1">
                    <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-violet-700 self-start mb-2">
                      {article.category}
                    </span>

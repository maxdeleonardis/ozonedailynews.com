...
const DEFENSE_TAGS = new Set([
  'defense', 'military', 'dod', 'pentagon', 'space force', 'air force',
  'navy', 'army', 'marines', 'national security', 'missile', 'hypersonic',
  'satellite', 'surveillance', 'intelligence', 'cyber', 'warfare', 'weapons',
  'contract', 'lockheed', 'raytheon', 'northrop', 'boeing defense',
  'general dynamics', 'darpa', 'gps', 'reconnaissance', 'drone', 'uav',
]);

function isDefenseArticle(slug: string, tags: string[]): boolean {
  if (slug.startsWith('/defense/')) return true;
  const lowerTags = tags.map((t) => t.toLowerCase());
  return lowerTags.some((t) => DEFENSE_TAGS.has(t));
}

export const metadata: Metadata = {
  title: 'Defense | Military Space, National Security & Contracts',
  description: 'Defense technology news covering military space programs, national security, DoD contracts, and defense industry analysis.',
  ...
  twitter: {
    ...
    title: 'Defense | Military Space, National Security & Contracts',
    description: 'Defense tech — military space, national security, and DoD industry analysis.',
  },
};
...
export default async function DefensePage() {
  ...
  const articles = all
    .filter((e) => e.lifecycle !== 'pruned' && isDefenseArticle(e.slug, e.tags ?? []))
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <nav className="text-xs text-gray-500 mb-4">
            ...
            <span className="text-black font-semibold">Defense</span>
          </nav>
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-slate-700 shrink-0 mt-1" />
            <div>
              <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-black">Defense</h1>
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">
                Military space programs, national security, DoD contracts, and defense technology analysis.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        {articles.length === 0 ? (
          <div className="py-24 text-center">
            ...
            <Link href="/" className="mt-4 inline-block text-sm text-slate-700 hover:underline">← Back to home</Link>
          </div>
        ) : (
          <>
            {hero && (
              ...
              >
                {hero.imageUrl ? (
                  ...
                  <div className="w-full sm:w-80 aspect-[16/9] sm:aspect-auto bg-slate-50 border-r border-slate-200 flex items-center justify-center shrink-0">
                    <span className="text-slate-400 text-4xl font-black opacity-40">◈</span>
                  </div>
                )}
                <div className="p-5 sm:p-6 flex flex-col justify-center flex-1">
                  ...
                  <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-slate-700 self-start mb-3">
                    {hero.category}
                  </span>
                  ...
                  {grid.map((article) => (
                    ...
                    >
                      {article.imageUrl ? (
                        ...
                        <div className="w-full aspect-[16/9] bg-slate-50 border-b border-slate-200 flex items-center justify-center shrink-0">
                          <span className="text-slate-400 text-3xl font-black opacity-30">◈</span>
                        </div>
                      )}
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-slate-700 self-start mb-2">
                          {article.category}
                        </span>
